...
def LR(data, label, alpha=0.01, iternum=500, method='BGD', sample_size=5):
    n, m = data.shape
    ...
    if method == 'BGD':
        ...
        for i in range(iternum):
            random_index = np.random.choice(n, sample_size)
...
if __name__ == '__main__':
    ...
    data[:, 1] = data[:, 1] / 5
    theta = LR(data, label, 0.01, 500, method='N')
